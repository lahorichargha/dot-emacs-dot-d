...
(defcustom blog-server-pass nil "Weblog server password. If this is nil you'll be prompted." :group 'blog :type 'string) 
...
(defconst blog-version "0.3" "Current version of blog.el")
...
;; muse-mode style for <example> tags to be rendered using <pre> without
;; any CSS class/style.
(muse-derive-style "blog-html"
                   "xhtml"
                   :strings '((begin-example . "<pre>")))
...
(defun blog-post-entry(&optional publish)
  ...
  (let* (r1 r2 html-text list-headers)
    (save-excursion
      ...
      (setq r2 (point-max))
      (end-of-line)
      (muse-publish-region r1 r2 (cdr (assoc "Subject" list-headers)) (muse-style "blog-html"))
    ...
    (if (looking-at "**Categories")
		(progn
		  ...
		  (let ((word-match (or (current-word t) ""))
			...
			(when word-match
			  (setq completion-match
					(completing-read "Category ? " blog-categories-list nil nil word-match))
